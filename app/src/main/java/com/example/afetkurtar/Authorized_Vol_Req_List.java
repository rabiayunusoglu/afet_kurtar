...
public class Authorized_Vol_Req_List extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_authorized__personel__list);
        drawerLayout = findViewById(R.id.per_list);
    ...
    protected void onRestart() {
        ...
        ((LinearLayout) findViewById(R.id.auth_lay_perlistScroll)).removeAllViews();
        list3 = new ArrayList<JSONObject>();
        getData1("");
        list2 = new ArrayList<JSONObject>();
        getData("");
    ...
    public void getData(String ID) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest
                (Request.Method.GET, "https://afetkurtar.site/api/volunteerUser/read.php", obj, new Response.Listener<JSONObject>() {
    ...
    public void addtext(JSONObject obj) {
        ...
        try {
            ...
            for (int x = list.size() - 1; x >= 0; x--) {
                try {
                    ...
                    if (!(tmp.getString("requestedSubpart").equals("0")) && tmp.getString("responseSubpart").equals("0")) {
        ...
        if (list2 != null)
            for (JSONObject x : list2) {
                ...
                LinearLayout scroll = findViewById(R.id.auth_lay_perlistScroll);
                ...
                try {
                    ...
                    if(x.getString("isExperienced").equals("0"))
                        deneyim="Yok";
                    else if(x.getString("isExperienced").equals("1"))
                        deneyim="Var";
                    if(x.getString("haveFirstAidCert").equals("0"))
                        egitim="Yok";
                    else if(x.getString("haveFirstAidCert").equals("1"))
                        egitim="Var";
                    String subname="",disname="";
                    for(JSONObject y : list3){
                        if(y.getString("subpartID").equals(x.getString("requestedSubpart"))){
                            disname=y.getString("disasterName");
                            subname=y.getString("subpartName");}
                    }
                    linear.setText("ID: " + x.getString("volunteerID") + "\n" + "İsim : " + x.getString("volunteerName") + "\n" + "İstek Afet Bölgesi : " + disname + "\n" + "İstek Alt Parçası: " + subname + "\n" + "Deneyim : " +deneyim + "\n" + "İlk yardım eğitimi : " +egitim);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void getData1(String ID) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest
                (Request.Method.GET, "https://afetkurtar.site/api/subpart/read.php", obj, new Response.Listener<JSONObject>() {
    ...
    public void addtext1(JSONObject obj) {
        String cevap = "";
        String deneyim= "", egitim= "";
        ArrayList<String> list = new ArrayList<String>();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.HistoryText:
                ...
                for (JSONObject x : list2) {
                    try {
                        if (x.getString("volunteerID").equals(tmp.trim())) {
                            disasterID = Integer.parseInt(x.getString("volunteerID"));
                            disasterName = x.getString("volunteerName");
                            disasteremail = x.getString("address");
                            disasterrole = x.getString("isExperienced");
                            disasterTeamid = x.getString("haveFirstAidCert");
                            emergencyins = x.getString("requestedSubpart");
                            emergencyins = x.getString("responseSubpart");
